...
download_and_extract_backhaul() {
    ...
    case "$ARCH" in
        x86_64)
            DOWNLOAD_URL="https://raw.githubusercontent.com/jenaze/backhaul/refs/heads/main/backhaul_amd64.tar.gz"
            ;;
        arm64|aarch64)
            DOWNLOAD_URL="https://raw.githubusercontent.com/jenaze/backhaul/refs/heads/main/backhaul_arm64.tar.gz"
            ;;
        *)
...
read_option() {
    echo -ne "Enter your choice [0-7]: "
    read choice
    choice=$(echo "$choice" | tr -d '[:space:]')
    
    if [[ ! "$choice" =~ ^[0-7]$ ]]; then
        colorize red "Invalid option! Please enter a number between 0 and 7." bold
        sleep 1
        return
    fi
